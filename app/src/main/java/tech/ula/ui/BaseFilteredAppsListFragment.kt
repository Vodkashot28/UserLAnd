...
abstract class BaseFilteredAppsListFragment : Fragment(), AppsListAdapter.AppsClickHandler {
    ...
    protected abstract fun categoryFilter(apps: List<App>): List<App>

    private fun filtered(query: CharSequence?): List<App> {
        val byCategory = categoryFilter(allApps)
        if (query.isNullOrBlank()) return byCategory
        val q = query.toString().lowercase()
        return byCategory.filter { it.name.lowercase().contains(q) }
    }

    private val appsObserver = Observer<List<App>> {
        it?.let { list ->
            allApps = list
            appsAdapter.updateApps(filtered(binding.searchApps.query))
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.searchApps.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            ...
            override fun onQueryTextChange(newText: String?): Boolean {
                appsAdapter.updateApps(filtered(newText))
                return true
            }
        })
    }
    ...
    private fun showAppDetails(app: App): Boolean {
        ...
        this.findNavController().navigate(R.id.app_details_fragment, bundle)
        return true
    }
